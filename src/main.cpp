...
#if defined(NDEBUG)
# error "Stepcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Stepcoin Signed Message:\n";
...
{
    int64_t nSubsidy = Params().PoWReward();
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (nActualSpacing > Params().TargetSpacing() * 4)
        nActualSpacing = Params().TargetSpacing() * 4;
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    LogPrintf("SetBestChain: new best=%s  height=%d  tx=%lu  trust=%s  blocktrust=%d  date=%s\n",
      hashBestChain.ToString(), nBestHeight, (unsigned long)pindexBest->nChainTx,
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    txCoinStake.nTime &= ~Params().StakeTimestampMask();
